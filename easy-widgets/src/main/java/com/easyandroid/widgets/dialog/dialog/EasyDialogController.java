...
import com.easyandroid.widgets.ScreenUtil;
...
/**
 * package: com.easyandroid.widgets.dialog.dialog.EasyDialogController
 * author: gyc
 * description:
 * time: create at 2022/7/3 22:42
 */
public class EasyDialogController {
    ...
    EasyDialogController(IDialog dialog) {
    ...
    private void dealDefaultDialog(IDialog.OnClickListener positiveBtnListener, IDialog.OnClickListener negativeBtnListener, final String titleStr, final String contentStr,
        ...
        if (showBtnRight && showBtnLeft) {
            ...
            if (btn_ok != null) {
                btn_ok.setVisibility(View.VISIBLE);
                btn_ok.setBackgroundResource(R.drawable.easydialog_lib_ui_selector_btn_border_bg);
                btn_ok.setText(Html.fromHtml(TextUtils.isEmpty(positiveStr) ? "确定" : positiveStr));
                btn_ok.setOnClickListener(mButtonHandler);
            }
        } else if (showBtnLeft) {
            ...
            if (btn_cancel != null) {
                btn_cancel.setVisibility(View.VISIBLE);
                btn_cancel.setBackgroundResource(R.drawable.easydialog_lib_ui_selector_btn_border_bg);
                btn_cancel.setText(Html.fromHtml(TextUtils.isEmpty(negativeStr) ? "取消" : negativeStr));
                btn_cancel.setOnClickListener(mButtonHandler);
            }
        }
